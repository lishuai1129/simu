<template>
  ...
  >
    ...
    <div class="bottom-right-tools">
      <!-- 星座控制按钮 -->
      <el-tooltip :content="useConstellationStore().simulationRunning ? '停止星座仿真' : '启动星座仿真'" placement="left">
        <el-button
          :icon="useConstellationStore().simulationRunning ? VideoPause : VideoPlay"
          circle
          :type="useConstellationStore().simulationRunning ? 'danger' : 'success'"
          @click="useConstellationStore().simulationRunning ? handleStopConstellation() : handleStartConstellation()"
        />
      </el-tooltip>
      <!-- 底图模式切换按钮 -->
...
import { Monitor, Close, Minus, Picture, View, VideoPlay, VideoPause } from '@element-plus/icons-vue';
...
const handleStartConstellation = () => {
  ...
  setTimeout(() => {
    if (constellationWs.getConnectedStatus()) {
      ...
      ElMessage.warning("WebSocket未连接成功");
    }
  }, 1500);
};
...
.bottom-right-tools .el-button:hover {
  background: rgba(0, 198, 255, 0.2);
  border-color: #00eaff;
  box-shadow: 0 0 15px rgba(0, 198, 255, 0.5);
}

.bottom-right-tools .el-button.is-primary {
  background: rgba(0, 198, 255, 0.3);
  border-color: #00eaff;
}

.bottom-right-tools .el-button.el-button--success {
  background: rgba(103, 194, 58, 0.2);
  border-color: #67c23a;
  color: #67c23a !important;
}

.bottom-right-tools .el-button.el-button--success:hover {
  background: rgba(103, 194, 58, 0.4);
  border-color: #67c23a;
  box-shadow: 0 0 15px rgba(103, 194, 58, 0.5);
}

.bottom-right-tools .el-button.el-button--danger {
  background: rgba(245, 108, 108, 0.2);
  border-color: #f56c6c;
  color: #f56c6c !important;
}

.bottom-right-tools .el-button.el-button--danger:hover {
